...
void BST::DeleteVertex(elem_t E)
{
 ...
 if((E == Root->Elem) && ((Root->Left == NULL)||(Root->Right == NULL)))// ** if what you want to delete is the root with one child 
  {  if(Root->Left == NULL)
      {
	Root = Root->Right;
	return;
	...
	Root = Root->Left;
	return;
      }//** change the root to the child and return - this could be the left or the right child 
...
void BST::remove(Vertex *V, Vertex *P)
{
  if((V->Left == NULL) && (V->Right == NULL))// ** if V is a leaf (case 1)
	...
	if(V < P) //(You need if then else to determine left or right)
	  {
	    P->Left = V->Left;
	    delete V;
	  }
  ...
  else if((V->Right != NULL) && (V->Left == NULL))// ** if V has just the right child
      {
    ...
    else if((V->Left != NULL) && (V->Right != NULL))//else // V has two children (case 3)
